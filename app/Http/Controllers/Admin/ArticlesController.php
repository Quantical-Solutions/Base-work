...
class ArticlesController extends Controller
{
    ...
    public function index()
    {
        ...
        $results = Article::select('articles.id', 'articles.title_fr', 'articles.published', 'articles.img_1', 'articles.content_fr', 'articles.created_at', 'categories.name AS category')->join('categories', 'articles.category_id', '=', 'categories.id')->where('categories.parent', 2)->orderBy('id', 'desc')->get()->toArray();
        $count = count($results);
        ...
        return view('admin/tools/scrud', ['title' => 'Articles', 'scrud' => $scrud]);
    }

    public function mode($mode, $id = false)
    {
        return view('admin/tools/edit-read', ['title' => 'Article']);
    }
}
